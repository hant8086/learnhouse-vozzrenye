...
async def get_active_user_summary(
    ...
    Resolves the org plan and its member limit via get_plan_limit. In non-SaaS
    mode the limit is treated as unlimited (no overage).
    """
    ...
    if _is_non_saas():
        ...
        plan = _get_org_plan(org_config) if org_config is not None else "free"
        plan_limit = get_plan_limit(plan, "members")
