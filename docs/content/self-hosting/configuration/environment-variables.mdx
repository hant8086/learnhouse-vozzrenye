...
| `LEARNHOUSE_TINYBIRD_API_URL` | Tinybird regional API URL. Required together with both runtime tokens; alone it leaves analytics unconfigured. | URL | unset | API |
| `LEARNHOUSE_TINYBIRD_INGEST_TOKEN` | Runtime token restricted to append access on the `events` datasource. | secret string | unset; required for analytics configuration | API |
| `LEARNHOUSE_TINYBIRD_READ_TOKEN` | Runtime token restricted to SQL/query read access. | secret string | unset; required for analytics configuration | API |

Tinybird analytics is fail-safe: all three variables must be non-empty. Resource creation/deployment uses a separate admin credential and is not an application runtime permission.
